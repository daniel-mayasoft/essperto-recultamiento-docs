...
48. **El adaptador lo elige un resolvedor: por la conexión de la oferta al invitar, por el proveedor
    del candidato al consultar, y con EvaluaTest como respuesta cuando nada lo dice** (2026-09-14,
    decidido con el usuario; paso 4, en dos partes). Es la pieza que la 38 mandó a esta etapa, y hace
    código la 6 y la 41.

    **La regla, en orden:**

    1. **Al guardar la oferta**, se congela **la conexión que mande el portal**; si no manda ninguna
       —el portal de hoy—, la que diga la regla de la empresa (3). Ya no se busca siempre la de
       EvaluaTest.
    2. **Al invitar**, por la conexión congelada en la oferta. Sin ella —oferta vieja o creada desde
       administración—, por la empresa (3). Y en el candidato se escribe **el proveedor que resolvió**.
    3. **Por la empresa**: una sola conexión → esa; ninguna → *sin conexión* y la etapa se salta (40);
       **dos → EvaluaTest.**
    4. **Al consultar resultados**, por el proveedor guardado en el candidato (6); vacío → EvaluaTest
       (15). El cron agrupa por proveedor dentro de la oferta: quien fue invitado en EvaluaTest se sigue
       consultando allí aunque la empresa haya cambiado.

    🔴 **Por qué «dos → EvaluaTest» y no un error.** Conectar PsicoAlianza es una acción en *Mi
    compañía*; si eso hiciera fallar el arranque en las ofertas que nadie tocó, cada candidato nuevo
    de una oferta de julio saldría descartado con alerta a soporte sin que la reclutadora hubiera
    cambiado nada en ella. EvaluaTest no es adivinar: es lo que esa oferta ya venía usando. El cambio
    a PsicoAlianza lo hace alguien a propósito, oferta por oferta, con el selector del paso 6. **El
    precio**: hasta ese selector, ninguna oferta se activa con PsicoAlianza salvo escribiendo la
    conexión a mano, que es lo que ya se hace en local.

    **Las llamadas fijas** que quedan —el enlace de respaldo del arranque, las pruebas adicionales del
    veredicto, la validación y el estado de sesión del 5b— pasan por el resolvedor o quedan
    condicionadas al proveedor. Las pruebas de la vacante para el portal siguen siendo de EvaluaTest
    (4). **El modo demo no pasa por el resolvedor**: sigue siendo condicional en el embudo (38).

    **En dos partes**: 4a —resolvedor, guardado de la oferta, arranque y cron—, que ya deja pasar a
...
51. **La oferta conserva su conexión al guardar, se elige a propósito cuando hay dos, y nada que no
    ...
    **Lo decidido:**
    ...
    - ✅ **Los textos del aviso de una vacante de PsicoAlianza, aprobados por el usuario**. Los de
      ...
      | No usable, por un motivo que no está en esta tabla (red para los que PsicoAlianza añada) | «Esta vacante ya no sirve en PsicoAlianza para invitar candidatos. Elige otra vacante.» (propuesto por el ejecutor en el 6.2b, aprobado por el usuario el 2026-09-15) |

    - ✅ **Una oferta activa cuya conexión congelada ya no está, en una empresa con otra conexión, se puede
      reparar desde la ficha** (2026-09-15, opinión previa del 6.2b, texto aprobado por el usuario): el
      aviso va junto a los controles, no en su lugar, y *Configurar* abre sin conexión ni vacante. Solo sin
      ninguna conexión sustituye a los controles, con el texto de hoy. El texto nuevo: *«Esta oferta tiene
      la prueba psicométrica activada, pero la conexión que usaba ya no está en tu empresa. La etapa se
      está omitiendo: configura otra conexión o desactiva la prueba.»* Paso 6.2b.
...
54. **La sesión se renueva cada hora si hace falta, la pasada del cron nunca la espera, y guardar la
    ...
    **«Soporte» es el buzón de los desarrolladores** (`ALERT_SUPPORT_EMAILS`), el que recibe los fallos
    ...
    | Conexión guardada, sin sesión y sin ningún intento todavía | «Pendiente de conexión — pulsa Conectar para conectar ahora.» (añadido el 2026-09-16 al escribir el 2c.3: no promete un intento automático en un momento dado) | Encendido |
...
56. **En el modal de conexión de *Mi compañía* el nombre no se edita, y la contraseña guardada no se
    ...
    parte de la 41 que obligaba a reteclear.

    **Lo que se vio:** antes de la rama el modal abría con la contraseña de EvaluaTest rellena, porque el
    backend la mandaba descifrada al navegador de cualquier usuario de la empresa. La rama cerró esa fuga
    (41) y el modal pasó a pedirla siempre vacía: cambiar el correo o el nombre exigía teclearla otra vez.
    El usuario lo encontró con demasiada fricción.

    | Qué | Decidido | Descartado |
    | --- | --- | --- |
    | **Nombre** | **Solo lectura** en el modal. Hoy es casi decorativo —una conexión por proveedor— y editarlo solo añadía fricción | — |
    | **Contraseña** | **Vacía, con puntos y el aviso «Déjala vacía para conservar la contraseña guardada.»**, y **obligatoria solo si la conexión no está configurada**. Si se deja vacía, la validación usa la que la empresa del usuario autenticado tiene guardada para ese proveedor, y el guardado la conserva (eso ya lo hacía desde la 41) | **Volver a enseñarla**: reabre la fuga. **Puntos como valor del campo**: guardar sin tocarla mandaría los puntos como contraseña nueva |

    **Y de paso, pedido por el usuario el mismo día: la pestaña del flujo de *Mi compañía* sigue un solo
    patrón**, el que ya tenía la verificación de cumplimiento: «**nombre** — Configurado — detalle» o
    «**nombre** — Sin configurar», y todos los botones dicen **Editar**. Aplica a los portales de empleo,
    las listas de exclusión, la prueba psicométrica y el cumplimiento. En la prueba psicométrica
    desaparece el nombre de la conexión, y su botón deja de decir *Conectar* sin conexión, para no
    confundirse con el *Conectar* de la sesión de PsicoAlianza (2c.3).
    ...
    **Lo que no da:** ver la contraseña guardada. Quien la consultaba en el modal tiene que tenerla en
    ...
    controlador y control negativo; portal con tipos limpios. Lo que ve el reclutador solo lo comprueban
...
El 6 se parte en **6.1 y 6.2**, con punto y no con letra, para no confundirlos con los pasos 6a y
6b de la etapa 1, que la bitácora cita.
...
| 2c.1 | ✅ **HECHO y revisado el 2026-09-15.** La pieza que consigue la sesión, sin que nadie la llame: Chromium por el puerto de proxy, clasificación de cada intento, reintentos dentro de una llamada, guardado en el almacén (decisiones 43 y 52) — brief `brief-etapa3-paso2c-1-acunador-de-sesion.md` (el nombre del archivo conserva la palabra vieja para no romper los enlaces). Verificado por el planificador: 121 suites y 1.234 pruebas (1.225 pasan, 9 omitidas), con la caché limpia; ninguna afirmación existente quitada; nadie la llama. En la revisión se arreglaron el navegador que quedaba huérfano al vencer el tiempo límite y el mensaje de error que se perdía. ✅ **Comprobación real hecha el 2026-09-16 en el servidor de pruebas**, por el código de producción: un intento, `passed` en 36,1 s, con ventana y por proxy móvil (detalle y lo que queda abierto en `before-deploy.md` §5). ~~Falta la comprobación real, que pasa al servidor de pruebas: el modo con ventana entró dos veces, pero siempre con el script de la prueba, nunca por este código~~ | Aditivo en código; una comprobación real contra la cuenta del cliente |
...
| 2c.3 | ✅ **Código HECHO y revisado el 2026-09-16; casos a mano pendientes** en `pruebas-a-mano.md`, que el usuario corre con la rama entera antes de desplegar. Brief `brief-etapa3-paso2c-3-boton-conectar.md`, con la opinión previa y dos rondas de revisión. Verificado por el planificador: backend 123 suites y 1.290 pruebas (1.281 pasan, 9 omitidas), con la caché limpia; portal con tipos limpios; una afirmación existente relajada a propósito (la respuesta de estado crece) y cubierta por la prueba nueva. En la revisión se arreglaron el sondeo que seguía vivo al salir de la pantalla, la consulta final al tope de doce minutos y una prueba que no mordía. ⚠️ **Anotado para el 7d**: guardar la conexión podría no enseñar «Conectando…» si la consulta llega antes de que arranque la ráfaga. 🔴 **Defecto visto en pruebas el 2026-09-16**: con la sesión sin responder, la etiqueta dice «Conexión fallida — vuelve a intentarlo.» **aunque el último intento haya entrado**, porque «hubo algún intento» cuenta también los que entraron; invita a gastar un intento contra la cuenta del cliente. Sin arreglo todavía. El 2026-09-16 la fila de la conexión también cambió de forma: la etiqueta va en la misma línea y *Conectar* junto a *Editar*. La etiqueta de *Mi compañía* elige entre los textos de la 54 con lo que la ruta de estado dice además de `status` (ráfaga en curso, bloqueada, tope, algún intento); una ruta *Conectar* que pide sesión al 2c.2 con motivo propio, saltando la espera y sin liberar bloqueo ni saltar tope; sondeo del estado solo mientras hay ráfaga en curso. El texto de la conexión sin ningún intento, aprobado por el usuario el 2026-09-16 (54). «Conectado» gana a la ráfaga en curso, corregido en la opinión previa. Casos a mano a `pruebas-a-mano.md`. Después del 2c.2 | Visible; backend aditivo y una ruta nueva |
